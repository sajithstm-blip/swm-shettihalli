...
const App = () => {
    ...
    try {
      if (authMode === 'login') {
        ...
        try {
            ...
            showStatus('success', 'Staff account registered.');
        } catch (regErr) {
            // Failsafe: if email exists, try logging in instead of hanging
            if (regErr.code === 'auth/email-already-in-use') {
                await signInWithEmailAndPassword(auth, cleanEmail, passInput);
                showStatus('success', 'Existing account verified.');
            } else {
                throw regErr;
            }
        }
        
        // Background bootstrap for admin
        if (cleanEmail === 'sajith.tm@saahas.org') {
            ...
            } catch (e) { console.warn("Background role sync pending."); }
        }
      }
    } catch (error) {
      console.error(error);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      try {
        if (u) {
          ...
          // --- FAILSAFE BYPASS: Immediate Access for Admin ID ---
          if (email === 'sajith.tm@saahas.org') {
             setUserRole('manager');
             setRoleLoading(false);
             setLoading(false);
             return; // Exit early to avoid DB fetch logic
          }
  ...
  const handleExportCSV = () => {
    ...
    link.download = `Saahas_SWM_Report.csv`;
    link.click();
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      ...
      <p className="font-bold text-slate-500 tracking-tight uppercase text-[10px]">Initializing Saahas SWM Tracker...</p>
    </div>
  );
        ...
        {emailInput.toLowerCase() === 'sajith.tm@saahas.org' && authMode === 'register' && (
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-3">
                ...
                <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest leading-relaxed text-left">
                    Root Administrator detected. Account creation will grant full operational control.
                </p>
            </div>
        )}
      </div>
  ...
  if (userRole === 'unauthorized') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl text-center space-y-6">
        ...
        <h1 className="text-2xl font-black text-slate-800">Authorization Pending</h1>
        <p className="text-slate-500 font-medium">Your account <b>{user.email}</b> is authenticated but not yet whitelisted for data access. Please contact <b>sajith.tm@saahas.org</b>.</p>
        <button onClick={handleLogout} className="text-blue-600 font-bold hover:underline flex items-center gap-2 mx-auto"><LogOut size={16}/> Return to Login</button>
      </div>
    </div>
  );
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><h1 className="text-3xl font-bold tracking-tight text-slate-800">Operational Dashboard</h1><p className="text-slate-500 font-medium">India SWM performance tracking</p></div>
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1 flex justify-between tracking-widest"><span>Blocks</span> {dashBlocks.length > 0 && <button onClick={() => setDashBlocks([])} className="text-blue-500 text-[9px] hover:underline">Clear</button>}</label>
