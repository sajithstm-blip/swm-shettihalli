...
const App = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        // Force role in DB if registering as the primary admin
        ...
        showStatus('success', 'Account registered.');
      }
    } catch (error) {
      console.error(error);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      try {
        if (u) {
          ...
          // Instant bypass for the specified Admin ID to prevent hang
  ...
  const saveConfig = async (newConfig) => {
    ...
    try {
      ...
      showStatus('error', 'Error publishing changes.');
    }
  };
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      ...
      <p className="font-bold text-slate-500">Initializing Saahas Tracker...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl space-y-8 animate-in fade-in">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 mt-2 font-medium">India Operational Portal</p>
        </div>
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input type="email" placeholder="email@saahas.org" required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" value={emailInput} onChange={e => setEmailInput(e.target.value)}/>
            </div>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Secret Password</label>
            <div className="relative">
          ...
          <button type="submit" disabled={authLoading} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-blue-700 transition shadow-xl shadow-blue-100">
            ...
            {authMode === 'login' ? 'Enter Dashboard' : 'Register Account'}
          </button>
        </form>

        <div className="text-center">
          <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="text-sm font-bold text-blue-600 hover:underline">
            {authMode === 'login' ? "New user? Create an account" : "Have an account? Log in instead"}
          </button>
        </div>
        ...
        {emailInput.toLowerCase() === 'sajith.tm@saahas.org' && authMode === 'register' && (
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl text-[10px] font-bold text-amber-700 text-center">
                ROOT ADMIN DETECTED: Registering with this ID will activate full permissions.
            </div>
        )}
      </div>
    </div>
  ...
  if (roleLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      <Loader2 className="animate-spin text-blue-600" size={40}/>
      <p className="font-bold text-slate-500 text-center">Identity Verified.<br/>Checking staff permissions for {user.email}...</p>
    </div>
  );
  ...
  if (userRole === 'unauthorized') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl text-center space-y-6">
        ...
        <p className="text-slate-500 font-medium">Your email <b>{user.email}</b> is not authorized. Contact <b>sajith.tm@saahas.org</b> to be whitelisted.</p>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><h1 className="text-3xl font-bold tracking-tight text-slate-800">Operational Dashboard</h1><p className="text-slate-500 font-medium">National performance overview</p></div>
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Scale</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
                ...
                <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none">HH Area</p><p className="text-2xl font-black text-slate-800 leading-tight">{statsAgg.covered.toLocaleString()}</p></div>
              </div>
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
                <div className="bg-indigo-50 text-indigo-600 p-4 rounded-2xl"><Truck size={28}/></div>
                <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none">Participation</p><p className="text-2xl font-black text-slate-800 leading-tight">{(statsAgg.covered > 0 ? (statsAgg.giving/statsAgg.covered*100).toFixed(1) : 0)}%</p></div>
              </div>
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
                <div className="bg-green-50 text-green-600 p-4 rounded-2xl"><CheckCircle2 size={28}/></div>
                <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none">Efficiency</p><p className="text-2xl font-black text-slate-800 leading-tight">{(statsAgg.giving > 0 ? ((statsAgg.seg / statsAgg.giving) * 100).toFixed(1) : 0)}%</p></div>
              </div>
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
                <div className="bg-red-50 text-red-600 p-4 rounded-2xl"><Ban size={28}/></div>
                <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none">Absences</p><p className="text-2xl font-black text-slate-800 leading-tight">{statsAgg.absences}</p></div>
              </div>
            </div>
...
// AUTO-RENDER FOR DEPLOYMENT (Ensures app mounts on Vercel)
