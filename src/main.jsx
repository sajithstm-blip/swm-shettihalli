...
const App = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      console.error(error);
      const msg = error.code === 'auth/user-not-found' ? 'User not found. Try Registering.' : 
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      try {
        if (u) {
          ...
          // CRITICAL: Hardcoded Admin Bypass to prevent authentication hangs
  ...
  const updateUserRole = async (email, role) => {
    ...
    try {
      ...
      showStatus('success', `Permission assigned: ${email} as ${role}`);
    } catch (e) {
  ...
  const removeSupervisor = (id) => {
    ...
    showStatus('success', 'User removed from system list.');
  };
  ...
  const submitDailyLog = async () => {
    ...
    try {
      ...
      showStatus('success', 'Entry submitted successfully.');
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl space-y-8 animate-in fade-in">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 mt-2 font-medium">Operations Management Portal</p>
        </div>
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input type="email" placeholder="name@saahas.org" required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" value={emailInput} onChange={e => setEmailInput(e.target.value)}/>
            </div>
          </div>
          <div className="space-y-1">
          ...
          <button type="submit" disabled={authLoading} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-blue-700 transition shadow-xl shadow-blue-100">
        ...
        <div className="text-center">
          <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="text-sm font-bold text-blue-600 hover:underline transition">
            {authMode === 'login' ? "New staff member? Create account" : "Already have an account? Log in"}
          </button>
        </div>
        ...
        {emailInput.toLowerCase() === 'sajith.tm@saahas.org' && authMode === 'register' && (
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl text-[10px] font-black text-amber-700 text-center uppercase tracking-widest leading-relaxed">
                Primary Administrator Detected: Registering will unlock full management access.
            </div>
        )}
      </div>
    </div>
  ...
  if (roleLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      <Loader2 className="animate-spin text-blue-600" size={40}/>
      <p className="font-bold text-slate-500 text-center tracking-tight uppercase text-[10px]">Authenticated.<br/>Syncing permissions for {user.email}...</p>
    </div>
  );
  ...
  if (userRole === 'unauthorized') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl text-center space-y-6">
        ...
        <p className="text-slate-500 font-medium">Identity verified, but your email is not authorized for data access. Contact <b>sajith.tm@saahas.org</b> for whitelisting.</p>
        <button onClick={handleLogout} className="text-blue-600 font-bold hover:underline flex items-center gap-2 mx-auto"><LogOut size={16}/> Sign Out & Try Different ID</button>
      </div>
    </div>
  );
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><h1 className="text-3xl font-bold tracking-tight text-slate-800">Operational Dashboard</h1><p className="text-slate-500 font-medium">Operational metrics & KPIs</p></div>
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Ward</label>
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1 flex justify-between"><span>Blocks</span> {dashBlocks.length > 0 && <button onClick={() => setDashBlocks([])} className="text-blue-500 text-[9px] hover:underline">Clear</button>}</label>
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">View Type</label>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                ...
                <div className="p-6 bg-red-50 border border-red-100 rounded-[32px] text-center"><p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-1">Gaps Recorded</p><p className="text-4xl font-black text-red-600">{stats.absences}</p></div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'review' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-6">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div><h1 className="text-3xl font-black text-slate-800">Raw Data Audit</h1><p className="text-slate-500 font-medium">Full log auditing</p></div>
                    ...
                    {reviewTableData.map((l, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-blue-50/20 transition">
                          ...
                          <td className="px-6 py-5"><div className="font-black text-slate-700">{l.blockName}</div><div className="text-[10px] text-slate-400 font-bold lowercase tracking-tighter">{l.supervisorName}</div></td>
          ...
          <div className="max-w-xl mx-auto py-6 animate-in slide-in-from-bottom-6">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase text-center block tracking-widest">Report Date</label><input type="date" className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold outline-none text-center" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} /></div>
              </div>
              <div className="flex items-center gap-3 p-5 bg-red-50 rounded-3xl border border-red-100">
                ...
                <label htmlFor="noCollection" className="text-red-700 font-bold text-sm">Gaps: No Collection Activity Today</label>
              </div>
        ...
        {activeTab === 'setup' && (
          <div className="max-w-4xl mx-auto py-6 space-y-8 animate-in slide-in-from-bottom-6">
            ...
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 gap-4"><div className="flex flex-col sm:flex-row gap-2"><input id="new-user-email" placeholder="staff@saahas.org" className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold outline-none" /><select id="new-user-role" className="bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold outline-none"><option value="supervisor">Field Supervisor (FS)</option><option value="coordinator">Project Coordinator (PC)</option><option value="manager">Project Manager</option></select><button onClick={() => { const em = document.getElementById('new-user-email').value; const rl = document.getElementById('new-user-role').value; if(em) updateUserRole(em, rl); }} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-black">Authorize Staff</button></div></div>
            </section>
            {/* List Sections for Supervisors, States, Wards, Blocks */}
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black flex items-center gap-3"><Users className="text-blue-500"/> Operational Staff</h3><button onClick={() => setConfig({...config, supervisors: [...config.supervisors, { id: Date.now().toString(), name: '' }]})} className="text-xs bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold">Add Lead</button></div>
            ...
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black flex items-center gap-3"><Globe className="text-blue-500" /> Regional Mapping</h3><button onClick={() => setConfig({...config, states: [...config.states, { id: Date.now().toString(), name: '' }]})} className="text-xs bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold">Add State</button></div>
            ...
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black flex items-center gap-3"><Database className="text-blue-500" /> Block Configuration</h3><button onClick={() => setConfig({...config, blocks: [...config.blocks, { id: Date.now().toString(), name: '', wardId: '', supervisorId: '', totalHH: 0 }]})} className="text-xs bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold">Add Block</button></div>
