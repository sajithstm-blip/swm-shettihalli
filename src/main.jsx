...
const App = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        // If it's the specific admin email, we'll force the role update
        if (emailInput.toLowerCase() === 'sajith.tm@saahas.org') {
            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'user_roles', emailInput.toLowerCase()), {
                role: 'manager',
                updatedAt: new Date().toISOString()
            });
        }
        showStatus('success', 'Account created! Please wait for role verification.');
      }
    } catch (error) {
      console.error(error);
      const msg = error.code === 'auth/user-not-found' ? 'User not found. Try Registering.' : 
                  error.code === 'auth/wrong-password' ? 'Incorrect password.' :
                  error.code === 'auth/email-already-in-use' ? 'Email already exists.' : error.message;
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      if (u) {
        ...
        setRoleLoading(true);
        try {
          // Hardcode Sajith as Manager
          if (u.email.toLowerCase() === 'sajith.tm@saahas.org') {
             setUserRole('manager');
          } else {
  ...
  const saveConfig = async (newConfig) => {
    ...
    try {
      ...
      showStatus('error', 'Failed to publish configuration.');
    }
  };
  ...
  const updateUserRole = async (email, role) => {
    ...
    try {
      ...
      showStatus('success', `Permission granted to ${email} as ${role}`);
    } catch (e) {
  ...
  const removeSupervisor = (id) => {
    ...
    showStatus('success', 'Staff removed from list.');
  };
  ...
  const submitDailyLog = async () => {
    ...
    try {
      ...
      showStatus('error', "Error saving log.");
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.download = `SWM_Tracker_Report.csv`;
    link.click();
    showStatus('success', 'CSV Downloaded.');
  };

  // --- Views ---
  if (loading) return <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4"><Loader2 className="animate-spin text-blue-600" size={40}/><p className="font-bold text-slate-500">Checking Auth Status...</p></div>;
      ...
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl space-y-8 animate-in fade-in">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <div className="space-y-1">
            ...
            <div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} /><input type="email" placeholder="email@saahas.org" required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" value={emailInput} onChange={e => setEmailInput(e.target.value)}/></div>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Password</label>
            <div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} /><input type="password" placeholder="••••••••" required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" value={passInput} onChange={e => setPassInput(e.target.value)}/></div>
          </div>
        ...
        <div className="text-center">
          <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="text-sm font-bold text-blue-600 hover:underline">
        ...
        {emailInput.toLowerCase() === 'sajith.tm@saahas.org' && authMode === 'register' && (
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl text-[10px] font-bold text-amber-700 text-center">
                ADMIN DETECTED: Registering with this email will grant full access.
            </div>
        )}
      </div>
    </div>
  ...
  if (roleLoading) return <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4"><Loader2 className="animate-spin text-blue-600" size={40}/><p className="font-bold text-slate-500 text-center">Authenticated.<br/>Verifying Permissions...</p></div>;
  ...
  if (userRole === 'unauthorized') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl text-center space-y-6">
        ...
        <p className="text-slate-500 font-medium">Email <b>{user.email}</b> is authenticated but has not been whitelisted. Please contact <b>sajith.tm@saahas.org</b> to enable your access.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 md:pb-0 flex flex-col md:flex-row">
      {statusMsg.text && (
        <div className={`fixed top-4 right-4 z-[100] p-4 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 ${statusMsg.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><h1 className="text-3xl font-bold tracking-tight text-slate-800">Operational Dashboard</h1><p className="text-slate-500 font-medium">SWM footprint & accountability</p></div>
            ...
            <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Ward Mapping</label>
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1 flex justify-between"><span>Blocks (Multi)</span> {dashBlocks.length > 0 && <button onClick={() => setDashBlocks([])} className="text-blue-500 text-[9px] hover:underline">Clear</button>}</label>
                ...
                <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-1">View</label>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                <h3 className="font-bold text-lg text-slate-700 flex items-center gap-3 mb-8"><Ban className="text-red-500" size={20}/> Failures</h3>
                <div className="p-6 bg-red-50 border border-red-100 rounded-[32px] text-center"><p className="text-[10px] font-black text-red-400 uppercase tracking-widest leading-none mb-1">Gaps Recorded</p><p className="text-4xl font-black text-red-600">{stats.absences}</p></div>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 space-y-6">
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Ward Mapping</label><select className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold outline-none" value={entryWard} onChange={(e) => {setEntryWard(e.target.value); setEntryBlock('');}}><option value="">Select Ward</option>{config.wards?.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>
                  <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Operating Unit</label><select className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold outline-none" value={entryBlock} onChange={(e) => setEntryBlock(e.target.value)}><option value="">Select Block</option>{config.blocks?.filter(b => b.wardId === entryWard).map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
                </div>
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase text-center block">Date</label><input type="date" className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 font-bold outline-none text-center" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} /></div>
              </div>
