...
const App = () => {
  ...
  const handleSendLink = async (e) => {
    ...
    setAuthLoading(true);
    // Use window.location.href to ensure the user returns to the exact URL they are currently on
    ...
    try {
      ...
      setIsLinkSent(true);
      showStatus('success', 'Authentication link sent! Check your inbox.');
    } catch (error) {
  ...
  useEffect(() => {
    // 1. Handle incoming login links
    if (isSignInWithEmailLink(auth, window.location.href)) {
      let email = window.localStorage.getItem('emailForSignIn');
      if (!email) {
        email = window.prompt('Please confirm your @saahas.org email address to complete sign-in');
      }
      if (email) {
        signInWithEmailLink(auth, email, window.location.href)
          ...
          .catch((err) => {
            ...
            showStatus('error', 'Sign-in link is invalid or has expired.');
          });
      }
    }

    // 2. Monitor User State and Role Persistence
  ...
  const saveConfig = async (newConfig) => {
    ...
    try {
      ...
      showStatus('success', 'Operational configuration published!');
    } catch (e) {
  ...
  const submitDailyLog = async () => {
    ...
    try {
      ...
      showStatus('success', `Log submitted for ${block.name}`);
  ...
  const timeSeriesData = useMemo(() => {
    ...
    // Chronological sort prevents Recharts tooltip duplication
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl space-y-8 animate-in fade-in">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 mt-2 font-medium">Field Operations Portal</p>
        </div>
        ...
        {!isLinkSent ? (
          ...
          <div className="p-6 bg-green-50 border border-green-100 rounded-3xl text-center space-y-4 animate-in zoom-in-95">
            ...
            <p className="font-bold text-green-800 leading-tight">Link sent to <b>{emailInput}</b>. Check your inbox to complete sign-in.</p>
            <button onClick={() => setIsLinkSent(false)} className="text-xs font-bold text-green-600 underline">Try a different email</button>
          </div>
        )}
      </div>
    </div>
  ...
  if (userRole === 'unauthorized') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[40px] shadow-2xl text-center space-y-6">
        ...
        <p className="text-slate-500 font-medium">Email <b>{user.email}</b> has not been authorized by the Project Manager.</p>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><h1 className="text-3xl font-bold tracking-tight text-slate-800">SWM Intelligence</h1><p className="text-slate-500 font-medium">Performance monitoring control panel</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
                ...
                <div><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none">Service Area</p><p className="text-2xl font-black text-slate-800 leading-tight">{statsAgg.covered.toLocaleString()}</p></div>
              </div>
              <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-5 transition hover:shadow-md">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                <h3 className="font-bold text-lg text-slate-700 flex items-center gap-3 mb-8"><History className="text-blue-500" size={20}/> Performance Trends</h3>
                <div className="h-80">
              ...
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                <h3 className="font-bold text-lg text-slate-700 flex items-center gap-3 mb-8"><Ban className="text-red-500" size={20}/> Gap Analysis</h3>
                {gapData.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-30"><CheckCircle2 size={64} className="text-green-500"/><p className="font-bold text-sm mt-3">No gaps recorded.</p></div>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'review' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-6">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div><h1 className="text-3xl font-black text-slate-800">Field Log Review</h1><p className="text-slate-500 font-medium">Detailed audit of operational entries</p></div>
        ...
        {activeTab === 'entry' && (
          <div className="max-w-xl mx-auto py-6 animate-in slide-in-from-bottom-6">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 space-y-6">
              ...
              <button onClick={submitDailyLog} className="w-full bg-blue-600 text-white py-5 rounded-[24px] font-black text-lg shadow-xl active:scale-[0.98] transition-all">Publish Entry</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'setup' && (
          <div className="max-w-4xl mx-auto py-6 space-y-8 animate-in slide-in-from-bottom-6">
            ...
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black flex items-center gap-3"><ShieldCheck className="text-blue-500"/> Account Access Control</h3></div>
            ...
            {/* Block Section */}
            <section className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black flex items-center gap-3"><Database className="text-blue-500" /> Blocks</h3><button onClick={() => setConfig({...config, blocks: [...config.blocks, { id: Date.now().toString(), name: '', wardId: '', supervisorId: '', totalHH: 0 }]})} className="text-xs bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold">Add Block</button></div>
